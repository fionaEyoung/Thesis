...
Brainstem tumours are often not candidates for surgical removal due to their precarious \note{/eloquent} location, with limited access and excessive risk to vital brainstem function.
...
These tumours can embed themselves insidiously within the brain's functional architecture with devastating prognosis, facing \note{/challenging} oncologists and surgeons with stark dilemmas in their bid to maximise both patient survival and quality of life.
Gliomas are classified into four \gls{who} \note{malignancy} grades, commonly split into \gls{lgg} (\gls{who} grades 1-2) and \gls{hgg} (\gls{who} grades 3-4) to reflect differences is aggressiveness and prognosis.
...
Complete removal of all pathological tissue, perhaps counterintuitvely, is not always the surgical objective.
...
\footnote[2]{\gls{eor} is relevant to epilepsy surgery, although the terminology and calculations here are different. Epileptogenic centres cannot always be distinguished and measured on imaging, and functionally eloquent tissue may be the clear source of epileptic \note{activity}, and thus subject to removal.}
...
Estimates of \gls{eor} can also be thrown off \note{/compromised} by post-operative brain tissue shifting and obscuring the actual volume of resected tumour \autocite{Schucht2014a}, while microscopic tumour cell invasion means that complete resection, as viewed either on imaging or by intraoperative visual assessment, does not necessarily mean no tumour residue remains\autocite{Yordanova2017}.
...
In particular, maximal resection of \glspl{lgg} drastically reduces the risk of residual tumour evolving into \gls{hgg} (known as malignant transformation \note{This is only true in adult LGG - in children WHO grade I LGG, which constitute the majority of LGG  never undergo malignant transformation})\autocite{Duffau2013,Hervey-Jumper2016,Rincon-Torroella2019}.
...
\note{this refer to adult tumours - mostly insular and tumours near the motor cortex. To balance this discussion you may want to mention awake craniotomies, which in adults helps to obtain a maximal safe resection. Obviously difficult in children.}
...
This strong demarcation between tumour and healthy tissue can facilitate surgical treatment as it is not necessary to risk functioning neural tissue while resecting infiltrating tumour, but such tumours can exert pressure on displaced tissues, leading to neurological impairments and a \note{reccommendation} for surgical debulking.
...
\note{I think this needs to be re worded. All tumours may cause an elevation of ICP, whether they are encapsulated or infiltrative; the elevation of ICP is the main need for surgery.}
...
Multi-contrast \gls{mri}, \note{for clinicians contrast is usually thought of as gadolinium} including FLAIR, T1w and T2w imaging sequences, each provide unique contrasts for visualising different aspects of a tumour, such as necrotic and infiltrating regions, which can aid in determining tumour type, what \gls{eor} to aim for, or which region of the tumour to target for biopsy.
...
\note{may be worth adding that these systems have been available since the mid 1990s and have developed and become more complex and accurate over time. One of the first references is Sandeman et al, 1995, which referred to the ‘wand’ - can look for the full reference if you need.}
...
\begin{SCfigure}[][h!]
  \includesvg[height=\textheight]{chapter_2/brain_shift.svg}
  \caption{Illustration of intraoperative brain shift and its effect on neuronavigation. Top: Preoperative imaging of a right \gls{who} grade 1 epidermoid lesion patient. Image is a $T_1$ weighted structural scan overlaid with \gls{csd}-derived \gls{dec} map. White arrowhead indicates medial displacement of the \gls{cst} (coloured blue/purple). Middle: Intraoperative imaging with partially resected lesion. Brain shift has caused the \gls{cst} to relax laterally towards the craniotomy (white arrowhead). Bottom: Streamline tractography reconstructions of the \gls{cst} from preoperative (red) and intraoperative (green) \gls{dmri}, with areas of overlap in yellow. Note how the red streamlines give the impression of a tract further from the resection margin.}
  \label{fig:shift}
\end{SCfigure}
