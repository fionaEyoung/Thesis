...
Solutions exist only in the form of specific \textit{eigenstates} of paired \textit{eigenfunctions} $\psi$ and associated \textit{eigenvalues} $a_n$ which give the possible values for a measured property.
Associated with each measurable property, such as total energy or position, is an operator $A$, which determines the possible measured values $a$ satisfying the eigenfunction equation

\begin{align}
  A \psi = a \psi
\end{align}

Solutions can take the form of eigenfunctions described by the quantum numbers $n,l,m_l$ and $m_s$, the latter of which we are now familiar with.

Where the eigenvalues are the possible energy states of the system.

Under a magnetic field, the eignestates $m_s = \pm \frac{1}{2}$ are no longer degenerate (of equal energy), but split into different energy levels, a phenomenon called the Zeemann effect.
...
\begin{align}
  \frac{N_{\uparrow}}{N_{\downarrow}} = e^{\Delta E / k T}, \label{eq:boltzmann}
\end{align}
where $\Delta E$ is the difference in energy between the two states.
A spin could be excited from the lower energy to higher energy state if absorbing a photon of a specific frequency: the Larmor frequency $\omega = \gamma B_0$.
...
The application of a strong linear magnetic field $\mathbf{B}_0$ along $z$ causes all spins in a sample to align themselves with the direction of that field, with the ratio of spins in each of the possible energy states determined by (\ref{eq:boltzmann}).
...
\begin{align}
  M_z(t)  &= M_0 (1-e^{-t/T_1}) \\
  M_xy(t) &= M_0 e^{-t/T_2} \label{eq:recovery}
\end{align}
...
In theory, these three encoding dimensions can apply to any permutation of the three spatial axes, but in practice, and for the purposes of this simplified description, it's easiest and conventional to select slices along the $z$-axis, parallel with the main magnetic field, and we will designate the $x$ and $y$ axes as the frequency and phase encoding dimensions respectively.
...
If differences in spin frequency can be likened to clocks running at different speeds, then two spins with different phases (but the same frequency) are like two clocks which both keep perfect time, except that one has not been set correctly and is perpetually 5 minutes behind the other (these two clocks would have a phase difference of 5 minutes).
...
The spatial frequencies and phases in this raw signal can be represented in the 2-dimensional frame known as $k$-space, an intermediate space defined as the inverse spatial \gls{2dft} of the measured image.
...
Any MR pulse sequence can be modified to introduce additional sensitisation to brownian motion, or diffusion weighting, in addition to the existing $T_1$ and $T_2$ based constrasts.
...
\note{missing section: epi artefacts, $T_2$ shine etc.}
