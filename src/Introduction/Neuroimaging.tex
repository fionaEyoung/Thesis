...
Diffusion \gls{mri} image \glspl{voxel} are on the order of cubic millimetres, and in such a volume are contained thousands of individual axons, with average diameters of around 1$\mu m$\autocite{Liewald2014,Lampinen2019}.
...
Streamline tracking proceeds virtually uninhibited, terminating only if a maximum length is reached or when leaving the \gls{wm} (as indicated by tissue segmentations\autocite{Smith2012} or \pgls{fod} amplitude threshold).
