...
\begin{SCfigure}[][h!]
  ...
  \caption{Example of potential for atlas misalignment. The \gls{af} (A) and tapetum (T) are proximal and parallel at the temporoparietal fibre intersection area. Linearly registered right \gls{af} atlas \glspl{tod} may overlap with the tapetum (arrowhead).}
  \label{fig:tpfia}
\end{SCfigure}
...
\begin{figure}
  ...
  \caption{Difference in tractfinder performance when using either affine or non-linear atlas registration, compared with targeted \gls{roi} tractography, in the \textit{TractoInferno} dataset of healthy subjects. For the \gls{dice} a threshold of 0.05 applies.}
  \label{fig:nrr}
\end{figure}
