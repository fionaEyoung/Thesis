...
\begin{figure}
    \includegraphics{compare_ntrain_.png}
...
A search on the online publication database Web of Science for \lstinline|(few OR single OR one) AND shot AND learning|, (excluding results about singl-shot echo planar imaging) revealed a sharply increasing trend in the broad field of medical imaging alone \note{as in, broadly defined} (Fig. \ref{fig:pubs}).

\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth]{pubs.png}
...
While the computational time spent on performing the prerequisite data augmentation training the network for these novel tracts is not disclosed in either \textcite{Liu2023a} or \textcite{Wasserthal2018}, with \note{3-500} epochs of batch size \note{162}, on a dataset of at least 6GB (before augmentation), one can assume \note{several hours} of training.
