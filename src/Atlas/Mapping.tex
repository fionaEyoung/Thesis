...
Both are represented in the modified \gls{sh} basis introduced in (\ref{sec:sh}):
...
Each of these direction samples is binned into a fixel based on its neighbourhood using a fast-marching level set algorithm, resulting in each fixel being represented by a collection of directional samples and the associated amplitudes of the original \gls{fod}.
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{Demonstration of fixel-wise inner product using the synthetic Fibre Cup Phantom. Crossing fibres results in low inner product signal for the brown tract in upper intersection region (b). In (c) this signal drop is eliminated resulting in smooth tract map.}\label{fig:fixip}
\end{figure}
