...
Even though parts of the precentral sulcus remain some distance from the resection site, the slight brain shift seems to have made fully recognising the \gls{cst} challenging for TractSeg. \note{less patronising?}
...
\begin{figure}[htb!]
  ...
  \caption{Intraoperative imaging for a paediatric temporal \note{tumour type} patient, with bilateral \gls{or} segmentations (thresholded according to Table \ref{tab:thresh}) shown in axial slices at 5mm intervals and as full volumes in the right-most column.}
  \label{fig:lb.gosh}
\end{figure}
