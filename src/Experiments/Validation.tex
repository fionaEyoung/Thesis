...
For example, in the \textit{TractoInferno} dataset and for the \gls{af}, mean bundle distance to reference tractography was 5.06~mm for the naive registered atlas and a very similar 5.07~mm TractSeg DKFZ at (Fig. \ref{fig:combobox}). % (Tab. \note{missing table tab:DATAHCP}).
...
\begin{figure}[h!]
  ...
  \caption{All methods compared against the \textit{TractoInferno} reference streamlines. \acrolist{af, cst, or, ifof, tf, at, tsx, tsd, tg}}
  \label{fig:combobox}
\end{figure}
...
\begin{SCfigure}
  ...
  \caption{Signed bundle distances for all methods compared against targeted tractography in the \gls{hcp} dataset. \acrolist{af, cst, or, ifof, tf, at, tsx, tsd}}
  \label{fig:hcpbox}
\end{SCfigure}
...
\begin{figure}[h!]
  ...
  \caption{\Gls{dice} and density correlation scores for tractfinder and TractSeg using the same training and testing streamlines from the HCP105 dataset. Mean over all subjects is shown for each tract. For abbreviations see \textcite{Wasserthal2018}}
...
\begin{SCfigure}[][htb!]
  ...
  \caption{Effect of deformation modelling on segmentation accuracy, compared with manually filtered targeted tractography. Each large datapoint represents the average across all tracts for a single subject, and is coloured according to the tumour side. Small datapoints represent individual tracts. In the one subject with a midline tumour, all tracts are considered ipsilateral.}
  \label{fig:btcd_def}
\end{SCfigure}
