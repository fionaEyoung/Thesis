...
\note{add a couple of sentences of overview}
...
We can also see that the signed bundle distance is significantly higher on average for the \gls{af} and \gls{ifof}, \note{what dataset / figure is this referring to??? boxplots for HCP using TG as reference?} meaning tractfinder typically under-segments these tracts relative to tractography, which is to be expected with large volumes streamlines fanning to adjacent cortical termini not included in the curated tract atlas.
...
Similarly, the \gls{ifof} reconstructions also display strong variability, owing in part to disagreements in its precise definition as reviewed in Section \ref{chap:atlas}.
...
We can see this particularly well in TractSeg's results when compared with reference tractography in the TractoInferno data, which was produced using the streamline clustering tool RecoBundlesX\autocite{Garyfallidis2018,Rheault2020a}.
In Figure \note{ref:boxplots hcp}, TractSeg scores notably high, compared to other methods, for the \gls{ifof} and \gls{af}, both association bundles whose terminations are poorly defined and/or difficult to constrain using tractography without obtaining cortical parcellations, and which consequently can be reconstructed with very extensive segmentations and large volumes (Fig. \note{ref:lightbox ifof and af}).
...
As a final word on reproducibility, we note that the results in Figure \note{fig:METRICSBOXPLOTS} are consistent with the comparisons between TractSeg and RecoBundles published in \textcite{Wasserthal2018}.
There, a mean \gls{dice} of between 0.58 and 0.67 across all tracts was reported.
Our measured \gls{dice}s between TractSeg (DKFZ) and TractoInferno reference tractography (which is based on RecoBundlesX\autocite{Garyfallidis2018}) range between 0.45 and 0.59 across the four tracts studied (Tab. \note{tab:DATATI}).
...
This is shown in Figure \ref{fig:ts_all_tracts}, analogous to Figure 6 in \textcite{Wasserthal2018}.
...
Unfortunately, using only two tissue compartments for \gls{msmt} \gls{csd} in this manner can produce inaccurate reconstructions at tissue boundaries, in particular a ``cannibalisation" of weak white matter signal in \gls{csf} partial volume voxels, leading to missing white matter \glspl{odf} in these areas.
This is the case for tracts like the anterior commissure (CA) and fornices (FX\_left, FX\_right) which are small structures in places directly adjacent to \gls{csf}.
