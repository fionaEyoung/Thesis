...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{chapter_4/all_metrics_tractoinferno+hcp.png}
...
Volumetric and distance metrics indicate consistent, if not always high, levels of agreement between tractfinder and compared techniques, TractSeg and tractography.

Tractfinder segmentations are also highly consistent, with comparison metrics with alternative methods varying by little across subjects (Tab. \note{tab:DATAHCP}).
...
\note{HCP}
Binary \glspl{dice} are low across the board for the arcuate fasciculus, owing to the dramatically different spatial extents of the segmentations.
...
Agreement is similarly high in the optic radiations, with slightly lower \glspl{dice} compared to the two TractSeg methods, which tend to include more thalamus and a lesser extent of Meyer's loop.
...
The density correlation helps illustrate the cases where the choice of threshold may have a disproportionate influence on subsequent binary comparisons.
...
\note{what else is there to say about this??}
