...
However, given the very large degree of variabiltiy between tractography algorithms and tract reconstruction approaches, even this is a poorly defined reference point.
...
\note{this section needs a different name/focus}

Qualitative results for a representative subject from the HCP dataset (identified as the only subject within the top 30 smallest deviations from the mean scores for all three of bundle distance, \gls{dice} and density correlation) are shown in Figures \ref{fig:lb.cstor}-\ref{fig:lb.afifo}.
...
\begin{figure}[h!]
  ...
  \caption{All methods compared against multi-ROI targeted tractography for the \gls{hcp} and \textit{TractoInferno} datasets combined (except TGR, which is \textit{TractoInferno} data only). \acrolist{af, cst, or, tf, at, tsx, tsd, tgr} \note{switch to square image}}
  \label{fig:combobox}
\end{figure}

\paragraph*{Performance and reproducibility}
\note{add condensed dice mat figure}
...
For example, in the HCP dataset and for the \gls{af}, mean bundle distance between the naive atlas and tractography was $5.45$mm and mean bundle distance between TractSeg DKFZ and tractography was very similar at $5.41$mm (Tab. \note{missing table tab:DATAHCP}).
...
We can also see that the signed bundle distance is significantly higher on average for the \gls{af} and \gls{ifof}, \note{what dataset / figure is this referring to??? boxplots for HCP using TG as reference?} meaning tractfinder typically under-segments these tracts relative to tractography, which is to be expected with large numbers of streamlines fanning to adjacent cortical termini not included in the curated tract atlas.
...
As a final comment on reproducibility, we note that the results in Figure \note{fig:METRICSBOXPLOTS} agree with values published in \textcite{Wasserthal2018}.
...
Our measured \gls{dice}s between TractSeg DKFZ and TractoInferno reference bundles (which is based on RecoBundlesX) range between 0.45 and 0.66 across the four tracts studied (Tab. \note{tab:DATATI}), demonstrating consistency between the two in different datasets.
...
Again, this is most apparent in the association tracts, where anatomical definitions differ widely (Fig. \note{fig:lb.afr}, \note{fig:lb.afl}).
...
Conversely in the \gls{cst}, which has a relatively well agreed-upon domain, segmentations have much higher volumetric agreement between methods. \note{need graphic to show this}
...
In Figure \note{ref:boxplots hcp}, TractSeg scores notably high against \note{the TractoInferno reference bundles}, for the \gls{ifof} and \gls{af}, both association bundles whose cortical terminations are poorly defined.
...
Overall, results in the clinical (GOSH \& NHNN) dataset were highly consistent with those seen in the healthy datasets.
...
Two example clinical subjects, one adult and one paediatric, are displayed in Figure \note{fig:lb.clin} and demonstrate key observations in the clinical applicability of the compared methods.
In Figure \note{fig:lb.nh}, a sagittal view displays the interaction between the surgical resection cavity of a frontal tumour and the \gls{cst}.
Tractfinder maps portions of the \gls{cst} in close proximity to the resection site, as does manual tractography, where the TractSeg segmentations are far more conservative, potentially missing \gls{cst} locations influenced by oedema, brain shift, or other tumour effects.
In Figure \note{fig:lb.gosh}, the anterior extent of Meyer's loop depicted by tractography is similarly included in the tractfinder segmentation, but absent from the TractSeg results.
...
Results were compared with manual targeted tractography with additional streamline filtering as described in Section \ref{sec:data_btcd}.
...
Figure \ref{fig:btcd_def} shows how deformation modelling changes segmentation results, measured with density correlation.
Although the sample size is small, deformation modelling improves segmentation accuracy in most cases, although there is considerable variation.
...
\note{Examples} in Figure \ref{fig:btcd_def_pat03} demonstrate how tractfinder including tumour deformation produces tract margins closer to those indicated by tractography than without deformation modelling, providing further evidence for the potential usefulness of tractfinder in neurosurgical applications.
