...
However, not only can the distortions around resection cavities be especially severe, neither can they be easily corrected for intraoperatively, as the current tools take on the order of tens of minutes to hours to run.

\note{figure of bad distortion!}
...
RESOLVE (readout segmentation of long variable echo-trains), a Siemens \note{product}, is one example of a commonly used \gls{rsepi} sequence.
...
In the end, we tested 3 different DTI sequences as well as several different 3-scan trace RESOLVE sequences (not discussed here).
