...
Using the same train - test data split as described in \note{??}, subsets of 15 and 30, as well as the full 63 training subjects were randomly selected, from which separate TOD atlases where constructed.
...
\begin{figure}
    ...
    \caption{Comparison of segmentation performance using different numbers of atlas training subjects. Results are grouped by tract, colour represents number of training subjects. AF = Arcuate Fasciculus, CST = Corticospinal tract, OR = Optic radiation}
    \label{fig:ntrain}
\end{figure}
...
The results indicate that additional atlas subjects beyond a minimum \note{what about lower??} number of 15 do little to nothing to improve tractfinder results.
This can be interpreted as the additional training subjects offering little in the way of additional information on inter-subject variability, as a lot of this variability is already smoothed out due to affine (instead of diffeomorphic) co-registration of training subjects into template space.
