...
In this example case of a NHNN patient with a tumour embedded within the right precentral gyrus (tumour histology information is unavailable for this subject) we will consider the combined effects of tumour mass effect and peri-tumoural oedema on tract reconstruction.
...
All three methods utilised the same \glspl{fod} as the basis for identifying the tract, which clearly depict the presence of \gls{wm} fibres within the oedematous zone, yet the combination of crossing \gls{af} fibres, altered diffusion, and \gls{cst} displacement presented a significant barrier to streamlines, with few penetrating to tract the \gls{cst} beyond.
...
Meanwhile, with the deformed atlas predicting the presence of fibres within the affected region and their expected orientation, both of which are confirmed by the \gls{fod} estimates, tractfinder \note{finish sentence}

\begin{figure}
  \includesvg[width=\textwidth,pretex=\sffamily,inkscapelatex=true]{chapter_6/oedema.svg}
